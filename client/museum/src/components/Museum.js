...
const Museum = () => {
  ...
  function buyTicket(id, number, ticketPrice) {
    navigate(
      `/user/ticket?id=${id}&name=${museum.name}&number=${number}&price=${ticketPrice}&date=${date}&time=4PM&latitude=${museum.latitude}&longitude=${museum.longitude}`
    );
  }
  ...
  const museumRender = (
    <div className="museumContainer">
      ...
      <div className="bookingCard">
        ...
        <br />
        <button onClick={() => buyTicket(id, number, ticketPrice)}>
          Purchase
        </button>
        {/* <p>{museum.latitude}</p>
  ...
  return <>{isLoading ? <h1>Loading...</h1> : museumRender}</>;
};
